import 'package:cross_mate/ui/camera_view_singleton.dart';
...
class BoxWidget extends StatelessWidget {
  ...
  const BoxWidget({super.key, required this.result, this.boxesColor, this.showPercentage = true});
  @override
  Widget build(BuildContext context) {
    ...
    if (boxesColor == null) {
      ...
      usedColor = Colors.primaries[((result.className ?? result.classIndex.toString()).length +
              (result.className ?? result.classIndex.toString()).codeUnitAt(0) +
              result.classIndex) %
    ...
    return Positioned(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
            border: Border.all(color: usedColor!, width: 3), borderRadius: const BorderRadius.all(Radius.circular(2))),
        child: Align(
