...
최적의 On-Device AI 모델을 선정하기 위해 체계적인 데이터 구축 및 모델 성능 비교/평가 과정을 거침.

### 1. 데이터 구축 및 분석 (Data)
- **데이터 수집**: `AI-Hub`의 공공 데이터, `Roboflow`의 오픈 데이터셋, 그리고 팀원들이 직접 촬영한 데이터를 모두 활용하여 총 5,310장의 최종 데이터셋 구축.
- **데이터 전처리**: 라벨링 오류를 직접 검수하고, 다양한 환경에서의 강인한 성능을 위해 데이터 증강 적용.

### 2. 모델 선정 및 평가 (Model)
- **후보 모델**: 모바일 환경에서의 성능을 고려하여 `YOLO`, `RT-DETR`, `SSD MobileNet V2`, `EfficientDet` 등 경량화된 모델들을 주요 후보로 선정.
- **성능 평가**: 테스트 데이터셋 기반의 mAP 평가와 실제 애플리케이션 탑재 후의 추론 속도(ms, FPS), GPU 사용량을 종합적으로 비교/분석.
- **최종 선정**: **YOLOv8n** 모델이 정확도, 속도, 리소스 효율성 모든 면에서 가장 뛰어난 성능을 보여 최종 모델로 선정. 양자화를 통해 추가적인 성능 개선도 확인함.
